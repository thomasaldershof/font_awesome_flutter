...
const List<String> ignoredAliases = ["try", "11ty"];
...
///     https://raw.githubusercontent.com/FortAwesome/Font-Awesome/7.x/otfs/Font%20Awesome%207%20Brands-Regular-400.otf
///     https://raw.githubusercontent.com/FortAwesome/Font-Awesome/7.x/otfs/Font%20Awesome%207%20Free-Regular-400.otf
///     https://raw.githubusercontent.com/FortAwesome/Font-Awesome/7.x/otfs/Font%20Awesome%207%20Free-Solid-900.otf
...
void adjustPubspecFontIncludes(Set<String> styles) {
  ...
  // If we find any font file that contains Pro we assume that the user wants to use the Pro
  // icon files. For that, we need to replace the font path in pubspec.yaml
  // from '...Free...otf' to '...Pro...otf'.
  var type = 'free'; // 'pro'
  final fontFiles = Directory('lib/fonts').listSync();

  if (fontFiles.any(
    (file) => file.path.split(Platform.pathSeparator).last.contains('Pro'),
  )) {
    type = 'pro';
  }
  ...
  for (var i = startFlutterSection; i < pubspec.length; i++) {
    ...
    if (styles.any((element) => element.replaceAll(' ', '') == styleName)) {
      final needsReplacement = styleName == 'regular' || styleName == 'solid';
      pubspec[i] = uncommentYamlLine(pubspec[i]);
      pubspec[i + 1] = uncommentYamlLine(pubspec[i + 1]);
      pubspec[i + 2] = uncommentYamlLine(
        needsReplacement
            ? replaceYamlLineByFontAwesomeType(pubspec[i + 2], type)
            : pubspec[i + 2],
      );
...
String replaceYamlLineByFontAwesomeType(String line, String type) {
  if (type == 'pro') {
    return line.replaceAll('Free', 'Pro');
  }

  return line.replaceAll('Pro', 'Free');
}
...
String generateIconDocumentation(IconMetadata icon, String style) {
  var doc =
      '/// ${style.sentenceCase} ${icon.label} icon\n'
      '///\n'
